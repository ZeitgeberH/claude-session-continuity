...
> - **Session-log chain location.** The chain lives *outside* the memory dir — precisely so the normalizer doesn't strip its `prev`/`next` links — but **in this project that means `<repo>/.claude/memory/session_logs/`**, alongside (not inside) the memory dir `<repo>/.claude/memory/store/`. `session-log-protocol.md` records the exact path; trust it over any path hardcoded here.
>   **Reorganized 2026-07-16 — older instructions in this file were wrong:** there is no longer a `.claude-memory` symlink (it pointed at an absolute *host* path that did not exist inside the devcontainer, so VS Code reported missing files). `~/.claude/projects/<proj>/memory` is now a symlink pointing *inward* at `store/`. Resolve the chain as `$CLAUDE_PROJECT_DIR/.claude/memory/session_logs`.
...
**Branch on whether the session-log chain is installed.** Resolve its location by probing, in order, `$CLAUDE_PROJECT_DIR/.claude-memory/session_logs` then `$CLAUDE_PROJECT_DIR/.claude/memory/session_logs` — the same two candidates the SessionStart hook uses. **In this project the chain is at the second path**, `<repo>/.claude/memory/session_logs/` (as of 2026-07-16; there is no `.claude-memory` symlink any more). It still lives OUTSIDE the memory dir (`store/`) so the frontmatter normalizer cannot strip its `prev`/`next` links — "outside the memory dir" is the invariant, not any particular path. `session-log-protocol.md` is authoritative. The chain is present if the resolved directory holds at least one `session_*.md`; a missing chain wrongly triggers the fallback, so probe both before concluding.
